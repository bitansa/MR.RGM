...
// Do MCMC sampling with Spike and Slab Prior for the model when X, Y and U are there and Sigma is full with IW prior on it
// [[Rcpp::export]]
Rcpp::List RGM_SpikeSlab_IW_Covariates(const arma::mat& S_YY, const arma::mat& S_YX, const arma::mat& S_XX,
                                       ...
                                       double nu_2 = 0.0001, double Prop_VarA = 0.01, double Prop_VarB = 0.01, double TAU = 2){
  ...
  // Make the diagonals of Gamma, Tau matrices to be 0
  Gamma.diag().zeros();
  Tau.diag().zeros();
    ...
    for (int j = 0; j < p; j++) {
      ...
      for (int l = 0; l < k; l++) {
        ...
        if (D(j, l) != 0) {
          ...
          // Sample b
          Rcpp::List Output1 = Sample_B_Full(S_YX, S_XX, S_XU, B, C, B, j, l, Sigma_Inv, MultMat, n, Phi(j, l),
                                             Eta(j, l), nu_2, Prop_VarB, -1, Trace1, Trace2, Trace3);
    ...
    for (int j = 0; j < p; j++) {
      ...
      for (int l = 0; l < p; l++) {
          ...
          if (l != j) {
            ...
            // Sample a
            Rcpp::List Output2 = Sample_A_Full(S_YY, S_YX, S_YU, A, A, j, l, Sigma_Inv, n, p, B, C, Gamma(j, l), Tau(j, l), nu_1,
                                               Prop_VarA, -1, Trace4, Trace5, Trace6, Trace7, Trace8, InvMat, logdet);
