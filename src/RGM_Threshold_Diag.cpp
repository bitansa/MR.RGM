...
// Do MCMC sampling with threshold prior for the model when both X and Y are there and Sigma is diagonal
// [[Rcpp::export]]
Rcpp::List RGM_Threshold_Diag(const arma::mat& S_YY, const arma::mat& S_YX, const arma::mat& S_XX, const arma::mat& D, double n, int nIter, int nBurnin,
  ...
  for(int i = 1; i <= nIter; i++){
    ...
    for (int j = 0; j < p; j++) {
      ...
      for (int l = 0; l < k; l++) {
        ...
        if (D(j, l) != 0) {
          ...
          // Sample b
          Rcpp::List Output1 = Sample_B_Diag(S_YX, S_XX, B, B_Pseudo, j, l, Sigma_Inv, MultMat, n, 1, Eta(j, l), nu_2, Prop_VarB, tB, Trace1, Trace2);
          double b = Output1[0];
    ...
    double Diff = LL_Diag(A, B_new, S_YY, S_YX, S_XX, Sigma_Inv, p, n) - LL_Diag(A, B, S_YY, S_YX, S_XX, Sigma_Inv, p, n)
    ...
    for (int j = 0; j < p; j++) {
      ...
      for (int l = 0; l < p; l++) {
        ...
        if (l != j) {
          ...
          // Sample a
          Rcpp::List Output2 = Sample_A_Diag(S_YY, S_YX, A, A_Pseudo, j, l, Sigma_Inv, n, p, B, 1, Tau(j, l), nu_1,
    ...
    // Calculate Difference
    double Diff_A = LL_Diag(A_new, B, S_YY, S_YX, S_XX, Sigma_Inv, p, n) - LL_Diag(A, B, S_YY, S_YX, S_XX, Sigma_Inv, p, n)
