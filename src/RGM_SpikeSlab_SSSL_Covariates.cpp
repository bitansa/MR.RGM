...
// Do MCMC sampling with Spike and Slab Prior for the model when X, Y and U are there and Sigma is full with spike and slab prior on it
// [[Rcpp::export]]
Rcpp::List RGM_SpikeSlab_SSSL_Covariates(const arma::mat& S_YY, const arma::mat& S_YX, const arma::mat& S_XX,
                                         ...
                                         double a_psi = 0.5, double b_psi = 0.5,
  ...
  for(int i = 1; i <= nIter; i++){
    ...
    arma::mat S = n * MultMat * S_YY * MultMat.t() - n * MultMat * S_YX * B.t() -
      ...
      + n * B * S_XU * C.t() + n * C * S_XU.t() * B.t() + C * C.t() / TAU;
