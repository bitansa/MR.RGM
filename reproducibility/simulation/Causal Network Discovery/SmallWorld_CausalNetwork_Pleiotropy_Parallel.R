...
# Run in parallel
results <- mclapply(1:num_sim, run_simulation, mc.cores = num_cores)
...
##
## Additional (pleiotropy case):
##   We also run the FullD / MR.RGM+ variant under the same SigmaStarModel = "SSSL"
##   setting to allow horizontal pleiotropy via a relaxed instrument structure.
###############################################################################
...
run_simulation_RGMCONF <- function(run) {
  ...
  metrics1_res1 <- metrics_binary_graph(
    as.vector((True_Graph))[off_diag_indices],
    as.vector((Output1$GammaEst > 0.5))[off_diag_indices] * 1
  )
  auc1 <- pROC::auc(pROC::roc(
    as.vector((True_Graph)[off_diag_indices]),
    as.vector((Output1$GammaEst)[off_diag_indices])
  ))
  ...
  metrics1_res2 <- metrics_binary_graph(
    as.vector((True_Graph))[off_diag_indices],
    as.vector((Output2$GammaEst > 0.5))[off_diag_indices] * 1
  )
  auc2 <- pROC::auc(pROC::roc(
    as.vector((True_Graph)[off_diag_indices]),
    as.vector((Output2$GammaEst)[off_diag_indices])
  ))
...
# Combine results
T_MR.RGM <- do.call(rbind, lapply(results, function(x) x$T1))
T_MR.RGM_FullD <- do.call(rbind, lapply(results, function(x) x$T2))

# Summary
colMeans(T_MR.RGM)
colMeans(T_MR.RGM_FullD)



# Get SD
apply(T_MR.RGM, MARGIN = 2, FUN = sd) / sqrt(nrow(T_MR.RGM)) * sqrt(nrow(T_MR.RGM) - 1)
apply(T_MR.RGM_FullD, MARGIN = 2, FUN = sd) / sqrt(nrow(T_MR.RGM_FullD)) * sqrt(nrow(T_MR.RGM_FullD) - 1)
...
##
## Additional (pleiotropy case):
##   We also run the FullD / MR.RGM+ variant under the same SigmaStarModel = "diagonal"
##   setting to allow horizontal pleiotropy while keeping independent residual errors.
###############################################################################
...
run_simulation_RGMNOCONF <- function(run) {
  ...
  ## =============== METHOD: MR.RGM without confounders ================
  ## Calculate S_YY, S_YX, S_XX
  S_YY = t(Y) %*% Y / n
  S_YX = t(Y) %*% X / n
  S_XX = t(X) %*% X / n
  ...
  # Run MR.RGM no confounder with full D
...
T_MR.RGM_NoConf <- do.call(rbind, lapply(results, function(x) x$T1))
T_MR.RGM_NoConf_FullD <- do.call(rbind, lapply(results, function(x) x$T2))


# Summary
colMeans(T_MR.RGM_NoConf)
colMeans(T_MR.RGM_NoConf_FullD)


# Get SD
apply(T_MR.RGM_NoConf, MARGIN = 2, FUN = sd) / sqrt(nrow(T_MR.RGM_NoConf)) * sqrt(nrow(T_MR.RGM_NoConf) - 1)
apply(T_MR.RGM_NoConf_FullD, MARGIN = 2, FUN = sd) / sqrt(nrow(T_MR.RGM_NoConf_FullD)) * sqrt(nrow(T_MR.RGM_NoConf_FullD) - 1)
