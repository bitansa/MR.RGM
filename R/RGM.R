...
#'
#' @param Model The model assumption parameter offers two options: "Model 1" and "Model 2", with a default value of "Model 1". "Model 1" assumes the model Y = AY + E, while "Model 2" assumes the model Y = AY + BX + E. If the user selects "Model 1", they must specify either Y or S_YY, along with n. Alternatively, if the user chooses "Model 2", they must specify either (X, Y) or (S_YY, S_YX and S_XX) or (S_XX, Beta, Sigma_Hat), along with d and n.
...
#' \item{Gamma_Pst}{An array containing the posterior samples of probabilities of edges between response variables in the graph structure.}
#'
#'
#'
#'
...
#'#' # -----------------------------------------------------------------
#' # Example 1:
#' # Run RGM based on individual level data with Threshold prior based on the model Y = AY + E
#'
#' # Data Generation
#' set.seed(9154)
#'
...
#' Output = RGM(Model = "Model 1", Y = Y, n = 10000, prior = "Threshold")
#'
...
#' # -----------------------------------------------------------------
#' # Example 2:
#' # Run RGM based on summary level data with Spike and Slab prior based on the model Y = AY + E
#'
#' # Data Generation
#' set.seed(9154)
#'
...
#' Output = RGM(Model = "Model 1", S_YY = S_YY,
...
#'
#' # Apply RGM on individual level data for Threshold Prior
#' Output = RGM(Model = "Model 2", X = X, Y = Y, d = c(2, 1, 1), n = 10000, prior = "Threshold")
#'
#' # Get the graph structure between response variables
#' Output$zA_Est
#'
...
#'
#' # Apply RGM on summary level data for Spike and Slab Prior
#' Output = RGM(Model = "Model 2", S_YY = S_YY, S_YX = S_YX, S_XX = S_XX,
...
#' # -----------------------------------------------------------------
#' # Example 5:
#' # Run RGM based on Beta and Sigma_Hat with Threshold prior based on the model Y = AY + E
#'
#' # Data Generation
#' set.seed(9154)
#'
...
#' Output = RGM(Model = "Model 2", S_XX = S_XX, Beta = Beta, Sigma_Hat = Sigma_Hat,
...
RGM = function(Model = c("Model 1", "Model 2"), X = NULL, Y = NULL, S_YY = NULL, S_YX = NULL, S_XX = NULL, Beta = NULL, Sigma_Hat = NULL, d, n, nIter = 10000, nBurnin = 2000, Thin = 1, prior = c("Threshold", "Spike and Slab"), a_rho = 3, b_rho = 1, nu_1 = 0.001, a_psi = 0.5, b_psi = 0.5, nu_2 = 0.0001, a_sigma = 0.01, b_sigma = 0.01, Prop_VarA = 0.01, Prop_VarB = 0.01){
  ...
  if (Model == "Model 1") {
    ...
    if(!is.null(Y)){
      ...
      # Check whether Y is both numeric matrix
      ...
      # Check whether number of rows of Y is equal to n
      if(nrow(Y) != n){

        # Print an error message
        stop("Number of rows of Y should be n.")
      ...
      # Print an error message
      stop("Please give Y or S_YY as input.")
    ...
    stop("Invalid Model parameter. Please specify either 'Model 1' or 'Model 2'.")
